...
const MarketSelect = (props) => {
    ...
    useEffect(() => {
        setFilteredData(searchText ? coins.filter(item => item.Code.toLowerCase().includes(searchText.toLowerCase()) || item.Name.toLowerCase().includes(searchText.toLowerCase())) : coins);
    }, [searchText]);

    useEffect(() => {
        setFilteredData(coins);
        setTrendingCoins(coins.filter(item => item.isTrending === true));
    }, [coins]);
            ...
            <View>
                <Pressable
                    ...
                    <View style={{
                        ...
                        <View>
                            <Text
                                style={[styles(activeTheme).title, maintenance && styles(activeTheme).line]}>{item.Code}</Text>
                            <Text
                                style={[styles(activeTheme).text, maintenance && styles(activeTheme).line]}>{item.Name}</Text>
                        </View>
        ...
        <>
            <View style={styles(activeTheme).wrapper}>
...

                {
                    loading ?
                        <ActivityIndicator
                            style={styles(activeTheme).loading}/> : filteredData && filteredData.length >= 1 ?
