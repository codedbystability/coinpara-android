    ...
    item.key === "ACCOUNT_APPROVE" ? <AccountApproveItem active={active} /> :
      <Pressable
        ...
        <View style={styles(activeTheme).leftWrapper}>
          ...
          <Text style={[styles(activeTheme, fontSizes).title]}>{getLang(language, item.title || item.key)}</Text>
        </View>
...
const styles = (props, fontSizes, active, isWrap) => StyleSheet.create({
  wrapper: {
    ...
    alignItems: "center",
    paddingVertical: DIMENSIONS.PADDING_BV / 1.4,
  ...
  wrapper2: {
    width: "100%",
    paddingVertical: DIMENSIONS.PADDING_H / 1.2,
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
    borderRadius: 8,
